...
Public Class OpCodesList
    ...
    Private Shared ReadOnly Property Table As Dictionary(Of OpCode, String)
        ...
        End Get
    End Property
    Public Shared Function GetAll() As List(Of MyOpcode)
        Dim AllList As New List(Of MyOpcode)
        For Each pair In Table
            Dim _toadd As New MyOpcode
            _toadd.Name = pair.Key.Name.ToString
            _toadd.Value = Conversion.Hex(pair.Key.Value)
            _toadd.Size = pair.Key.Size.ToString
            _toadd.Info = pair.Value
            _toadd.OpCodeType = pair.Key.OpCodeType.ToString
            _toadd.FlowControl = pair.Key.FlowControl.ToString
            _toadd.OperandType = pair.Key.OperandType.ToString
            _toadd.StackBehaviourPush = pair.Key.StackBehaviourPush.ToString
            _toadd.StackBehaviourPop = pair.Key.StackBehaviourPop.ToString
            AllList.Add(_toadd)
        Next
        Return AllList
    End Function
